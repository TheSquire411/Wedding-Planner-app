...
const FREE_TIER_MESSAGE_LIMIT = 5;
...
export default function ChatPage() {
  ...
  const isProUser = state.user?.tier === 'pro';
  const userMessageCount = state.chatMessages.filter(m => m.sender === 'user').length;
  const canSendMessage = isProUser || userMessageCount < FREE_TIER_MESSAGE_LIMIT;
  const remainingMessages = Math.max(0, FREE_TIER_MESSAGE_LIMIT - userMessageCount);
  ...
  useEffect(() => {
    ...
    if (state.chatMessages.length === 0) {
      const welcomeMessage: ChatMessage = {
        ...
        message: `Hi ${state.user?.name}! I'm your AI wedding planning assistant powered by Deepseek. I can help you with vendor recommendations, timeline planning, budget advice, and answer any wedding-related questions you might have. ${!isProUser ? `As a free user, you have ${FREE_TIER_MESSAGE_LIMIT} messages to get started. ` : ''}What would you like to know?`,
        sender: 'ai',
        timestamp: new Date()
      };
      dispatch({ type: 'ADD_CHAT_MESSAGE', payload: welcomeMessage });
    }
  }, [dispatch, state.chatMessages.length, state.user?.name, isProUser]);
  ...
  return (
    <>
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            {!isProUser && (
              <button
                ...
                className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-primary-500 to-sage-400 text-white rounded-lg hover:from-primary-600 hover:to-sage-500 transition-all"
              >
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-primary-100 text-sm">
                    Powered by Deepseek AI • {isProUser ? 'Unlimited messages' : `${remainingMessages} messages remaining`}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-gray-200 p-6">
              {!isProUser && (
                <div className="text-center text-sm mb-4">
                  <div className={`inline-flex items-center space-x-2 px-3 py-2 rounded-full ${
                    remainingMessages > 2 ? 'bg-green-100 text-green-700' :
                    remainingMessages > 0 ? 'bg-yellow-100 text-yellow-700' :
              ...
              <div className="flex space-x-4">
                <input
                  ...
                  placeholder={canSendMessage ? "Ask me about budgets, vendors, timelines, or anything wedding-related..." : "Upgrade to Pro to continue chatting..."}
      ...
      <UpgradeModal 
        ...
        featureName="Unlimited AI Assistant"
        description="Get unlimited access to our AI wedding planning assistant for personalized advice, vendor recommendations, and timeline planning."
      />
    </>
  );
}